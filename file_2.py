...
query_rank  = np.vectorize(query_rank,cache=True)
...
class Selected_Page(tk.Frame):
    def __init__(self, master):
        ...
        self.txtItem = tk.Text(self.frm, wrap='word', yscrollcommand=self.scrollbarV.set, xscrollcommand=self.scrollbarH.set,
...
class PageOne(tk.Frame):
    def __init__(self, master):
        ...
        button_display = tk.Frame(self)
        page_1_label = tk.Label(button_display, text="Output:")
        page_1_label.pack(side="top", fill="x", pady=10)
        i = 0
        for out in output:
            if i == 5 or out[1] == 0:
                break
            print(out[0])
            b = tk.Button(button_display, text=out[0], command=lambda: self.switch(master, out))
            b.pack(side="top", fill="x", padx=(100,100) ,pady=10)
            i += 1
        print(final_output)
        input = ""
        start_button = tk.Button(button_display, text="Back",
                                 command=lambda: self.BACK(master))
        start_button.pack(side="bottom")

        button_display.pack(side='left')

        #scrollv1=tk.Scrollbar(self,orient='vertical')
        #scrollh1 = tk.Scrollbar(self, orient='horizontal')
        scrollv2 = tk.Scrollbar(self, orient='vertical')
        scrollh2 = tk.Scrollbar(self, orient='horizontal')
        self.ans = tk.Label(self, relief='sunken', text=" the answer will come here" ,height=200 ,width=150)

        #scrollv2.grid(row=1,column=4,sticky='n'+'s')
        #scrollv2.config(command=self.ans.yview)

        #scrollh2.grid(row=2, column=0,columnspan=4, sticky='e'+'w')
        #scrollh2.config(command=self.ans.xview)
    ...
    def switch(self, master, out):
        ...
        str= text[6 * final_output[1][Selected_novel[0]]['index'] + 6]
